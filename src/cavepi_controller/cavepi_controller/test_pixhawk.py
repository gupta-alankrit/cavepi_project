...
class PixhawkLogicNode(Node):
    def __init__(self):
        super().__init__('test_pixhawk')
        ...
        self.pressure_publisher = self.create_publisher(
            msg_type=FluidPressure, topic='/pressure_data', qos_profile=10)
    ...
    def callback_depth_hold(self, msg):
        ...
        if self.is_depth_hold_started != self.previous_depth_hold_state:
        ...
        self.previous_depth_hold_state = self.is_depth_hold_started
        ...
        try:
            ...
            if msg:
                ...
                if msg_type == 'SCALED_PRESSURE2':
                    # self.get_logger().info(f"Pressure: {msg.press_abs:.2f} meters")
                    pressure_data = FluidPressure()
                    pressure_data.fluid_pressure = msg.press_abs
                    self.pressure_publisher.publish(pressure_data)
    ...
    def toggle_blink(self):
        if self.blink_count >= self.blink_limit:
            ...
            self.get_logger().info("Finished blinking. Lights are now ON.")
            return
